...
class picSearch:
    def search(self,query):
        ...
        self.url = "http://www.picsearch.com/index.cgi?q=%s"%(query.replace(" ","+"));
        self.sup = BeautifulSoup(get(self.url).content,'html.parser');
        for rst in self.sup.find_all('span',{'class':'result'}):
            data['page1']['height'].append(rst.a.img['data-height'])
            data['page1']['width'].append(rst.a.img['data-width'])
            data['page1']['image'].append('https:'+rst.a.img['src'])
            self.site = BeautifulSoup(get(rst.a['href']).content,'html.parser');
            for site in self.site.find_all('div',{'class':'detail-links'}):
                data['page1']['sites'].append(site.p.a['href']);
        return loads(dumps(data,sort_keys=True,indent=2));
